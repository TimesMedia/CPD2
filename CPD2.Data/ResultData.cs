...
namespace CPD2.Data
{
    ...
    public class ResultData
    {
        ...
        private static List<History> HistoryReader(ref DbCommand pCommand)
        {
            try { 
                List<History> lResults = new List<History>();

                using (DbDataReader dataReader = pCommand.ExecuteReader())
                {
                    while (dataReader.Read())
                    {
                        History lHistory = new History()
                        {
                            ResultId = (int)dataReader[nameof(History.ResultId)],
                            ModuleId = (int)dataReader[nameof(History.ModuleId)],
                            Publication = (string)dataReader[nameof(History.Publication)],
                            Issue = (string)dataReader[nameof(History.Issue)],
                            Module = (string?)dataReader[nameof(History.Module)],
                            Attempt = (int)dataReader[nameof(History.Attempt)],
                            URL = (string?)dataReader[nameof(History.URL)],
                            Date = (DateTime?)dataReader[nameof(History.Date)],
                            Score = (int?)dataReader[nameof(History.Score)],
                            Verdict = (string?)dataReader[nameof(History.Verdict)],
                            DateIssued = (DateTime?)dataReader[nameof(History.DateIssued)],
                            NormalPoints = (int?)dataReader[nameof(History.NormalPoints)],
                            EthicsPoints = (int?)dataReader[nameof(History.EthicsPoints)],
                            CustomerId = (int?)dataReader[nameof(History.CustomerId)],
                            Surname = (string?)dataReader[nameof(History.Surname)]
                        };
            ...
            catch (Exception ex)
            {
                ...
                {
                    ExceptionLevel++;
                    ExceptionData.WriteException(1, ExceptionLevel.ToString() + " " + CurrentException.Message, "static ResultData", "HistoryReader", "");
        ...
        public static List<History> GetCurrentTest(int pCustomerId)
        {
            try
            {
                DbProviderFactory factory = SqlClientFactory.Instance;
